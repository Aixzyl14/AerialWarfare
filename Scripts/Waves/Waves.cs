...
public class Waves : MonoBehaviour
{
    ...
    enum State { Fixed, MovingRight, MovingLeft };
    State state = State.Fixed;
    float leftX = 0f;
    float rightX = 0f;
    float MoveWaveRangeRight = 100f;
    float MoveWaveRangeLeft = 80f;
    ...
    private void FixedUpdate()
    {
        ...
        Vector3 WavePosOffset = new Vector3(-340, -1.5f, -800);
        Vector3 WaveNewPosition = BoatNewPosition + WavePosOffset;
        WaveNewPosition = AssigningWavePosition(WaveNewPosition);
    ...
    private Vector3 AssigningWavePosition(Vector3 WaveNewPosition)
    {
        if (WaveNewPosition.x >= MoveWaveRangeRight)
        {
            state = State.MovingRight;
        }
        if (WaveNewPosition.x <= MoveWaveRangeLeft)
        {
        ...
        switch (state)
        {
            ...
            case State.Fixed:
                WaveNewPosition.x = Mathf.Clamp(transform.position.x, leftX, rightX);
                break;

            case State.MovingLeft:
                leftX -= 50f;
                rightX -= 50f;
                MoveWaveRangeLeft -= 50f;
                MoveWaveRangeRight -= 50f;
                WaveNewPosition.x = Mathf.Clamp(transform.position.x, leftX, rightX);
                state = State.Fixed;
                break;
